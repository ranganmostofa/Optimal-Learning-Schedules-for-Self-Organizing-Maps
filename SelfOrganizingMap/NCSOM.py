...
class NCSOM:
    ...
    def train(self, input_map, learning_rate_schedule, neighborhood_radius_schedule, num_iter, beta=0.00001, C=100):
        """

        """
        self.__validate_input_data(input_map)

        N = int(np.prod(self.get_lattice_dimension()))
        winning_frequencies = np.zeros(self.get_lattice_dimension())
        losing_frequencies = np.zeros(self.get_lattice_dimension())
        ...
        for iteration_index in range(num_iter):
            ...
            # phase 1: competition without conscience
            ...
            y = np.zeros(winning_frequencies.shape)
            y[winner_index] = 1

            winning_frequencies = winning_frequencies + np.multiply(beta, np.subtract(y, winning_frequencies))

            # phase 2: competition with conscience

            bias_matrix = np.multiply(C, np.subtract(1.0 / N, winning_frequencies))

            winner_index = np.unravel_index(np.argmin(np.subtract(euclidean_distance_matrix, bias_matrix)), euclidean_distance_matrix.shape)
            ...
            y = np.zeros(losing_frequencies.shape)
            y[loser_index] = 1

            losing_frequencies = losing_frequencies + np.multiply(beta, np.subtract(y, losing_frequencies))

            # phase 2: competition with conscience

            bias_matrix = np.multiply(C, np.subtract(1.0 / N, losing_frequencies))

            loser_index = np.unravel_index(np.argmax(np.add(euclidean_distance_matrix, bias_matrix)), euclidean_distance_matrix.shape)

            # phase 3: cooperation

            # perform some computational geometry to determine learning rate scale
...
som = NCSOM(lattice_dimension, prototype_dimension)
